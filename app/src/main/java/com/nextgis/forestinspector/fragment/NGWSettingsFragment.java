...
import com.nextgis.forestinspector.activity.NGWSettingsActivity;
        ...
        implements OnAccountsUpdateListener
{
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        if (null == mAccountManager) {
            mAccountManager = AccountManager.get(mStyledContext.getApplicationContext());
        }
            ...
            String rootKey)
    {
        ...
        switch (action) {
            case SettingsConstantsUI.ACTION_PREFS_NGW:
                mAction = mActivity.getIntent().getAction();
                initializePreferences(screen);
                break;
        }
    }
    ...
    protected void initializePreferences(PreferenceGroup screen)
    {
        if (mAction != null && mAction.equals(ACCOUNT_ACTION)) {
            Account account = mActivity.getIntent().getParcelableExtra(KEY_ACCOUNT);
    ...
    protected void fillPreferences(PreferenceGroup screen)
    {
        if (null != mAccountManager) {
            for (Account account : mAccountManager.getAccountsByType(Constants.NGW_ACCOUNT_TYPE)) {
                ...
                Intent intent = new Intent(mStyledContext, NGWSettingsActivity.class);
            ...
            PreferenceGroup actionCategory)
    {
        ...
        {
            final ProgressDialog mProgressDialog = new ProgressDialog(mStyledContext);
